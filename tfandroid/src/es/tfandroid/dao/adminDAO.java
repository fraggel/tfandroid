...
public class adminDAO {
	...
	public boolean crearDescarga(int idmarca,int idmodelo,String titulo, String descripcion, String urlimagen,String idioma, boolean visible,String info,String features,String marcaModelo){
		ArrayList listaNoticias=null;
		Connection conn =null;
		try {
			Context initialContext = new InitialContext();
			...
			DataSource ds= (DataSource)(ctx.lookup("jdbc/tfandroid"));
            conn = ds.getConnection();
            CallableStatement calstm=conn.prepareCall("insert into downloads(idmarca,idmodelo,titulo,descripcion,urlimagen,idioma,visible,info,features,marcaModelo) values(?,?,?,?,?,?,?,?,?,?)");
			...
			calstm.setString(10, marcaModelo);
			calstm.executeUpdate();
			conn.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean modificarDescarga(String idmarca,String idmodelo,String titulo, String descripcion, String urlimagen,String idioma, String visible,String iddownload,String fecha,String info,String features,String marcaModelo) {
		ArrayList listaNoticias=null;
		Connection conn =null;
		boolean ret=true;
		try {
			...
			Context ctx=(Context)initialContext.lookup("java:comp/env");
            conn = ((DataSource)(ctx.lookup("jdbc/tfandroid"))).getConnection();
            CallableStatement calstm=conn.prepareCall("update downloads set titulo= ?,urlimagen=?,descripcion=?,idioma=?,visible=?,fecha=?,info=?,features=?,marcaModelo=? where idmarca=? and idmodelo=? and idioma=? and iddownload=?");
			calstm.setString(1, titulo);
			...
			calstm.setString(9, marcaModelo);
			calstm.setInt(10, Integer.parseInt(idmarca));
			calstm.setInt(11, Integer.parseInt(idmodelo));
			calstm.setString(12, idioma);
			calstm.setInt(13, Integer.parseInt(iddownload));
			calstm.executeUpdate();
	...
	public ArrayList consultaDescargas(){
		...
		try {
			...
			Context ctx=(Context)initialContext.lookup("java:comp/env");
            conn = ((DataSource)(ctx.lookup("jdbc/tfandroid"))).getConnection();
            CallableStatement calstm=conn.prepareCall("select iddownload,idmarca,idmodelo,fecha,titulo,descripcion,urlimagen,idioma,visible,marcaModelo from downloads order by fecha desc ");
			ResultSet set=calstm.executeQuery();
	...
	public ArrayList consultaDescargas(String idioma,int marca,int modelo){
		...
		try {
			...
			Context ctx=(Context)initialContext.lookup("java:comp/env");
            conn = ((DataSource)(ctx.lookup("jdbc/tfandroid"))).getConnection();
            CallableStatement calstm=conn.prepareCall("select iddownload,idmarca,idmodelo,fecha,titulo,descripcion,urlimagen,idioma,visible,info,features,marcaModelo from downloads where idioma= ? and idmodelo= ? and idmarca = ? order by fecha desc ");
			calstm.setString(1, idioma);
