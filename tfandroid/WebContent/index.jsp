...
%>
<div id="tableDescDivImg"><b><%=inicio.getMarcaModelo() %></b><br/><b><%=inicio.getTitulo()%></b><br/><p id="grey"><%=inicio.getFecha()%></p><br/><a href="ControlServlet?action=4&detalle=<%=inicio.getIdprincipal()%>&subDetalle=<%=inicio.getIdsecundario() %>&downloadId=<%=inicio.getIdterciario()%>"><img width="90%" src="<%=inicio.getUrlimagen()%>"/></a><br/><%if(inicio.getDescripcion().length()>200){%><%=inicio.getDescripcion().substring(0,200)%>...<%}else{%><%=inicio.getDescripcion()%><%}%><br/><br/><a href="ControlServlet?action=4&detalle=<%=inicio.getIdprincipal()%>&subDetalle=<%=inicio.getIdsecundario() %>&downloadId=<%=inicio.getIdterciario()%>"><%=RB.getString("home.readmore") %></a><br/><hr/></div>
<% }
	else{
		%>
